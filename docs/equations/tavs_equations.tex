...
On first encounter with block $m$ the variance is initialised from that round's
distances, subject to an absolute floor:
\begin{equation}
\sigsq_m^2 \leftarrow \max\!\left(1,\ \operatorname*{median}_{i \in \Vset} d_i^{(m)}\right).
\label{eq:bootstrap}
\end{equation}
The floor of $1$ is dimensionally arbitrary and is a defect in its own right:
once the model has converged, typical $d_i^{(m)}$ are of order $10^{-3}$, so the
initialisation overstates the true scale by three orders of magnitude and
\eqref{eq:z} is suppressed for the tens of rounds the EMA takes to decay.
